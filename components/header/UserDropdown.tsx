...
export default function UserDropdown() {
  ...
  useEffect(() => {
    async function getUser() {
      ...
      if (session?.user) {
        ...
        // Try to get display name from metadata or email fallback
        const displayName =
        ...
        // Optional: redirect to signin if no session
        // router.push("/signin");
      }
    ...
    // Listen for auth changes (e.g., sign in/out from other tabs)
    const {
      data: { subscription },
  ...
  // Show loading skeleton or fallback while fetching
  if (loading) {
    return (
  ...
  // If no user (not logged in), show nothing or a sign-in link
  if (!user) {
    return (
      <Link
        href="/signin"
        className="font-medium text-theme-sm text-brand-500 hover:text-brand-600"
      >
        Sign In
      </Link>
    );
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center text-gray-700 dark:text-gray-400 dropdown-toggle focus:outline-none"
